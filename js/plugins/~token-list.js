...
$.TokenList = function (input, url_or_data, settings) {
    ...
    // Initialization
    //
	if (!settings.width) settings.width = $(input).width() ? $(input).width() : 400;
    ...
    var input_box = $("<input type=\"text\"  autocomplete=\"off\">")
        ...
        .blur(function () {
            hide_dropdown();
            if (settings.bottomLink) {
            	//window.location=settings.bottomLink.replace('{val}', $(this).val());
            } else {
	            //$(this).val("");
            }
        })
            ...
            switch(event.keyCode) {
                ...
                case KEY.BACKSPACE:
                    ...
                    if(!$(this).val().length) {
                        ...
                        setTimeout(function(){do_search();}, 2000);
                    }
                    break;
                ...
                case KEY.COMMA:
                  if(selected_dropdown_item && !settings.disableEnter) {
    ...
    function delete_token (token) {
        ...
        if (token_data.id >= 0) saved_tokens = saved_tokens.slice(0,index).concat(saved_tokens.slice(index+1)); //UPDATED
        if(index < selected_token_index) selected_token_index--;

        // Update the hidden input
        update_hidden_input(saved_tokens, hidden_input);

        if (token_data.id >= 0) token_count -= 1; //UPDATED
    ...
    function update_hidden_input(saved_tokens, hidden_input) {
        var token_values = $.map(saved_tokens, function (el) {
            return (el.group ? el.group+'_' : '') + el[settings.tokenValue];
        });
    ...
    function show_dropdown() {
        var position = hidden_input[0].style.position || 'absolute';
    	...
    	dropdown
            .css({
                ...
                'max-height': $(document).height() - top - 25,
                overflow: 'auto',
                zindex: 999
            })
            .show();
	...
	settings.tokenFormatter = function(item) { 
		...
		item[this.propertyToSearch] = item[this.propertyToSearch].replace('Add: ','');
	...
	settings.onResult  = function(results) {
		if (settings.allowInsert) {
			var has_it = false;
			for (var i in results) if (results[i][settings.propertyToSearch] == $(input_box).val()) has_it = true;
			if (!has_it) {
				var item = {}
					item['id'] = 0;
	...
	if (settings.placeholder) {
		if (token_list.find('li.'+settings.classes.token).length > 0 && settings.linkedtext) {
			var value = {id:-2, name: settings.linkedtext, 'class':'linked-text-token'};
		} else {
			var value = {id:-2, name: settings.placeholder, 'class':'placeholder-token'};
		}
		settings.placeholder_token = insert_token(value);
	}
	
	//showDropdownOnFocus - can not be ommited its visual
	...
	input_box.focus(function() {
		...
		if (settings.showDropdownOnFocus || input_box.val()) { 
            setTimeout(function(){                             
		...
		if (settings.placeholder_token) return;
       	if (!(settings.tokenLimit === null || settings.tokenLimit !== token_count)) return;
		if (input_box.val()) return;
		
		if (settings.placeholder || settings.linkedtext) {
			if (token_list.find('li.'+settings.classes.token).length > 0 && settings.linkedtext) {
				var value = {id:-2, name: settings.linkedtext, 'class':'linked-text-token'};
			} else {
				var value = {id:-2, name: settings.placeholder, 'class':'placeholder-token'};
...
$.fn.initTokenInput = function() {
	...
	var default_attrs = ['type','name','data-url','onadd','ondelete','id','class','style','title','value']
    $('input.tokenInput:not(.initialized)').each(function() {
    	...
    	for(var i=0;i<attrs.length;i++) {
    		...
    		if (!is_def) {
    			//name = attrs[i].name.replace(/_(.)/gi,function(m){return m[1].toUpperCase()});
    	...
    	if ($(this).attr('value')) {
    		opts.prePopulate = eval($(this).attr('value'));
    	}
    	...
    	$(this).tokenInput(data_or_url, opts);
    });
}

...
function SendEmail($to, $subject, $message)
{
	...
	echo "Load $limit users at ".date('H:i:s Y-m-d')." ... ";
							
	$res = mysql_pquery("SELECT beta_id, signup_email
							FROM alpha_users 
							WHERE alpha_users.check = '0' AND time < DATE_SUB(NOW(), INTERVAL 8 HOUR)
	...
	while ($row = mysql_fetch_object($res)) {
		...
		SendEmail($email, $newsletter->subject, $newsletter->msg);

...
function formTests ( qForm, data ) {
	...
	console.warn(qForm + ' ' + data.submit);

	var $submit = $( qForm + ' ' + data.submit );
	var qError = data.error instanceof Function ? data.error : ( data.error ? data.error : null );
	var qSuccess = data.success ? data.success : null;
	...
	var data = {
		...
		inputs: [
			...
			},
			{
				...
				badValue: [ '', 'asd asd', 'qwe"qwe', 'io<div>sio' ],
				originalValue: user.uriName,
				validate: '#notification_bar p'
			}
		],
		submit: 'input[type="submit"]',
		error: '#notification_bar:not(.success) p',
		success: '#notification_bar.success'
...
testModule( 'Update your password', function () {
	...
	var data = {
		...
		inputs: [
			{
				...
				validate: '#notification_bar.error p',
				originalValue: user.password
			},
			{
				selector: 'input[name="new_pass"],input[name="re-new_pass"]',
				badValue: [ '', '12345' ],
				validate: '#notification_bar.error p',
				originalValue: user.password
			}
		...
		error: function ( qForm ) {
			return $( '#notification_bar.error p' ).is( ':visible' );
		},
		success: '#notification_bar.success'
	};
...
testModule( 'Email', function () {
	var data = {
		...
		submit: 'input[type="submit"]',
		error: '#notification_bar.error p',
		success: '#notification_bar.success'
	};

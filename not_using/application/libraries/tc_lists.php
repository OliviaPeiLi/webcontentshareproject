...
class tc_Lists {
    ...
    **/
	...
	function get_userinfo()
	{
			...
			{
					// friend name
				$data['friendname'][$r->uid] = addslashes($r->name);
				...
				$col++; if ( $col == 3 ) { $col = 0; $row++; }
					...
					if($r->avatar == '')
					{
							$data['avatar'][$r->uid] = s3_url().'pages/default/defaultInterest/'.$r->interest_id.'.png';;
					}
					else
					{
						$data['avatar'][$r->uid] = s3_url().$r->avatar;
					}
			...
			foreach ( $circle as $r )
			{
					$data['circle_element'][$r->cid] = $this->CI->Lists_database->get_friend_of_circle( $userid, $r->cid );
			...
			if ( $circle_order->num_rows() > 0 ) {
					...
					$this->CI->Lists_database->set_circle_order( $userid, $data['circle_order'] );
			}
	...
	function ajax_changeCircleName($cid, $newname)
	{
		...
		//	$newname = $this->CI->input->post('newname');
			...
			if ( $cid > 0 ) {
					...
					$this->CI->load->model('Lists_database');
					$this->CI->Lists_database->change_circle_name( $uid, $cid, $newname );
					$data['status'] = 1;
			} else {
					$data['status'] = 0;
			}
	...
	function ajax_createNewCircle($name, $element)
	{
			...
			if ( trim($name) == "" || $this->CI->Lists_database->circle_is_exist($uid, trim($name)) === true ) {
					...
					$data['status'] = 1;
					$data['circle_id'] = $this->CI->Lists_database->create_new_circle( $uid, trim($name) );

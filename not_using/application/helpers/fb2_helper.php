...
if (!function_exists('get_login_status')) {
   function get_login_status () {
		...
		if($cookie) {
			...
			$encoded = "access_token=".$cookie['access_token']."&message='I joined Jimmy Fandrop'";
			...
			echo "<h2>Facebook profile : </h2><br>";
			print_r($user);
			$user_id = $user->id;
			echo "<br><br>";
			$page = json_decode(file_get_contents('https://graph.facebook.com/me/likes?access_token=' . $cookie['access_token']));
			echo "<h2>Facebook pages which you like</h2><br>";
			print_r($page);
			echo "<br><br>";
			echo "<img src='https://graph.facebook.com/$user_id/picture?type=large'>";
		}
	}
}*/
function get_fb_feed($username)
{
	$facebook = new Facebook(array(
	  //'appId'  => '453074795120',
	  //'secret' => '1e599e77338d57c70b74d5d7be6cf513',
	...
	if ($session) {
	  try {
		$feed = $facebook->api('/'.$username.'/feed');
...
function getfbfriends()
{
	...
	if ($session) {
	  try {
		...
		{
			$friend_id = $friend['id'];
/*			$likes = $facebook->api('/'.$friend_id.'/likes');
			print_r($likes);*/
			$movies = $facebook->api('/'.$friend_id.'/movies');
			echo "Movies<br>";
			print_r($movies);
			echo "<br><br>";
			
			$music = $facebook->api('/'.$friend_id.'/music');
			echo "Music<br>";
			print_r($music);
			echo "<br><br>";

			$books = $facebook->api('/'.$friend_id.'/books');
			echo "Books<br>";
			print_r($books);
			echo "<br><br>";
			...
			$games = $facebook->api('/'.$friend_id.'/games');
			echo "Games<br>";
			print_r($games);
			echo "<br><br>";

			$television = $facebook->api('/'.$friend_id.'/television');
			echo "Television<br>";
			print_r($television);
			echo "<br><br>";
...
function do_like($page_id)
{
	...
	if ($session) {
	  try {
		...
		print_r($attachment);
		$result = $facebook->api('/161804483899366/likes',
                                'post',
                               $attachment );
		echo $result;
		echo "<br>liked";
...
function post_on_fb($username)
{
	$facebook = new Facebook(array(
	  //'appId'  => fb_app_key(),
	  //'appId'  => '115671641849040',
	  //'secret' => fb_app_secret(),
	  //'secret' => 'cab2200bdbf1a5a2e4df0bd6a1bdd9e0',
	  'appId'  => FACEBOOK_APP_ID,
	  'secret' => FACEBOOK_APP_SECRET,	  
	  'cookie' => true,
	));
	...
	if ($session) {
	  try {
		...
		$msg = array('message' => 'I just signed up for the beta of this cool new thing fandrop. You should check it out!',
                'name' => 'Fandrop',
//                'caption' => "Caption of the Post",
                'link' => 'http://www.fandrop.com',
//                'description' => 'this is fandrop description',
                'picture' => 'https://s3.amazonaws.com/fandrop/images/Encapsulated-Logo-for-FB.png',
...
*/			
			...
			$data = array('user'=>$user, 'page'=>$page, 'friends'=>$friends);
			return $data;
		}
	}
}
    ...
    function post_on_fb($username)
    {
    	...
    	if ($session) {
    	  try {
    		...
    		$me = $facebook->api('/me');
    		$likes = $facebook->api('/me/likes');
    		$attachment = array('message' => "I'm one of the first in line to explore a revolutionary online interest community with #fandrop. Join me http://fandrop.com @fantooninfo",
                    'name' => 'Fandrop',
                    'caption' => "",
                    'link' => 'http://www.fandrop.com',
                    'description' => 'Fandrop is an interest network that connects people through shared interests.',
                    'picture' => 'http://www.fandrop.com/images/fandropHeaderLogo.png',
                    'actions' => array(array('name' => 'Get Search',
                                      'link' => 'http://www.google.com'))
                    );

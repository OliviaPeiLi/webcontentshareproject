...
if ($page_id || ($view_type === 'home') || ($view_type === 'notification') || ($istopic === '1')) {
	$hierarchial_comments = true;
	//echo 'HELLO';
} else {
	$hierarchial_comments = false;
	//echo 'GOODBYE';
}
//Hack to fix the $view_type=topic defaulting to =notification
if ($istopic) $view_type = 'topic';
?>
	...
	<? 
		...
		foreach ($news_array['comments'] as $ck=>$cv)
		{
			...
			if($thumb == s3_url())
			{
				...
				{	
					$thumb = s3_url().'users/default/defaultMale_thumb.png';			
				}
			}

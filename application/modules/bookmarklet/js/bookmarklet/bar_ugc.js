...
define(["bookmarklet/communicator",  "plugins/jsend"], function(communicator) {
	...
	*/
	...
	communicator._onload = function(data, no_start) {
		...
		if (!no_start) {
			if ($('#bar a.video_mode').length) {
				console.info("INIT video mode");
				$('#bar a.video_mode').trigger('click');
			} else if ($('#bar a.image_mode').length) {
				console.info("INIT image mode");
				$('#bar a.image_mode').trigger('click');
			} else {
				console.info("INIT page mode");
				$('#bar a.drop_page').trigger('click');
			}
		}
	}
	...
	$(document).on('click', '#bar a.video_mode', function() {
		_gaq.push(['_trackPageview', 'external-scraper-video-mode-opened']);
		communicator.show_video_mode();
		
		//Mixpanel tracking
		if (typeof(mixpanel) !== 'undefined') {
			var user = php.userId ? php.userId : 0;
			mixpanel.people.identify(user);
			mixpanel.track('BOOKMARKLET: Video mode', {'user':user});
		}
		return false;
	});

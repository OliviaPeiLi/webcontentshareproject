/*
 * $ Plugin: Tokenizing Autocomplete Text Entry
 * Version 1.6.0
 *
...
var methods = {
    ...
    //init: function(url_or_data_or_function, options) {
        ...
        return this.each(function () {
        	try {
            	$(this).data("tokenInputObject", new $.TokenList(this, url_or_data_or_function, settings, default_data));
        	} catch (e) {
...
define(['jquery'], function() {
	// Expose the .tokenInput function to $ as a plugin
	...
	$.TokenList = function (input, url_or_data, settings, default_data) {

	//$.TokenList = function (input, url_or_data, settings) {
	    //
	    // Initialization
	    //
		if (!settings.width) settings.width = $(input).width() ? $(input).width() : 290;
	    ...
	    $(input).bind('pre_submit', function() {
	    	update_hidden_input(saved_tokens, input);
	    });
	    $(input).closest('form').submit(function() {
	    	$(input).trigger('pre_submit');
	    });
	    ...
	    var input_box = $("<input>").attr('type','text').attr('autocomplete','off')
	                ...
	                case KEY.DOWN:
	                    if(!$(this).val()) {
	                        ...
	                        if(event.keyCode === KEY.DOWN || event.keyCode === KEY.RIGHT) {
	                            dropdown_item = $(selected_dropdown_item).nextAll('li:first');
	                        } else {
	                            dropdown_item = $(selected_dropdown_item).prevAll('li:first');
	                        }
	                ...
	                case KEY.COMMA:
	                	if(!settings.disableEnter) {
	                		if (selected_dropdown_item) {
	                			var $target = $(selected_dropdown_item).data("tokeninput");
	    ...
	    if ($(input).val() && !settings.prePopulate) {
		    input_box.val($(input).val());
	    }
	    ...
	    var token_list = $("<ul />")
	        ...
	        .click(function (event) {
	        	// new $ doesn't trigger hidden elements
	    ...
	    try { //http://www.weavetexfashion.com/baby-frocks.html
		    token_list.css({'max-height': Math.max(24, $(document).height() - input_offset.top) });
	    } catch (e) {}
	    ...
	    */
	    ...
	    var doc_body = document.body.tagName != 'body' ? $('html') : $('body');
	    var dropdown = $("<div>")
	    ...
	    $(document).on('click', function(e) {
	    	if (   ! $(e.target).closest("div[class^='token-input-dropdown']").length 
	    	    && ! $(e.target).closest('li[class^="token-input-"]').length
	    	 ) {
	    		window.setTimeout(function() {

		    		console.info('hide on doc.click', e);
		    		hide_dropdown();
					if (dropDownCache) {
						//add_token(dropDownCache);
					}
	    ...
	    $(document).on('keydown','input.add-item-input', function(e) {
	    ...
	    this.destroy = function() {
	    	token_list.remove();
	    	$(input).nextAll('input.tokenInput-hidden').remove();
...
*/
	    ...
	    function update_hidden_input(saved_tokens, hidden_input) {
	    	...
	    	for (i=0;i<saved_tokens.length;i++) {
	    		...
	    		var new_input = $('<input type="hidden" name="'+input.name.replace('[]','')+group+'['+saved_tokens[i].id+']'+(saved_tokens[i].id?'':'[]')+'" class="tokenInput-hidden"/>');

	    			// error on validate
	                 var li_data = settings.prePopulate || hidden_input.data("pre");
	                 // fix bug with input
	                 if (li_data && li_data.length && li_data[0].id == undefined)	{
	    			   var sel_item = $('<option>').addClass("popular").val(0).attr("selected",true).html(saved_tokens[i].name);
	    			   $(input).append(sel_item);
	    			 }
	    ...
	    function show_dropdown() {
	    	...
	    	//console.info('show dropdown');
	       	var ptop = $(token_list).offset().top + $(token_list).outerHeight();
	       	if (dropdown.hasClass('fixed')) ptop -= $(window).scrollTop();
	   		var pheight = $(window).height() - ptop + $(window).scrollTop();
	   		try { //http://www.weavetexfashion.com/baby-frocks.html
		    	dropdown.css({
		                top: ptop,
	    ...
	    function position_dropdown() {
	    	...
	    	//the top position of the input_box
	   		...
	   		var pheight = $(window).height() - ptop;
	   		
	   		if (!dropdown.hasClass('fixed')) {
	   			pheight += $(window).scrollTop();
	   		}
	   		
	   		/*console.info('{position dropdown}', {
	   			'widow.height': $(window).height(),
	       	...
	       	if (pheight <= 150) {
	       		try { //http://www.weavetexfashion.com/baby-frocks.html
		       		dropdown.css('top', ptop - $(token_list).outerHeight() - dropdown.height());
		       		dropdown.css('max-height', ptop - $(token_list).outerHeight() );
		   		} catch (e) {}
	       	} else {
	       		try { //http://www.weavetexfashion.com/baby-frocks.html
		...
		settings.tokenFormatter = function(item) { 
			...
			container.html('<p>'+item[this.propertyToSearch]+'</p><span class="ico"></span>');
		...
		settings.onResult  = function(results) {
			if (settings.allowInsert === 'true' || settings.allowInsert === true) {
				...
				/*
				var has_it = false;
				if ($(input_box).val()) for (var i in results) if (results[i][settings.propertyToSearch] == $(input_box).val()) has_it = true;
				...
				if (!has_it && !popular_dropdown) {
						if ($(input_box).val()) {
							item[settings.propertyToSearch] =  'Add: '+$(input_box).val();
							//add_new_btn.show();
							selected_dropdown_item = null;
						} else {
							var item = {}
		...
		//var width = $(input).width() > 0 ? $(input).width() : 400;
	...
	$.fn.initTokenInput = function() {
	    ...
	    $('select.tokenInput:not(.initialized)').each(function() {
	    	var $self = $(this);
	    	...
	    	*/
			...
			if ($(this).attr("data-create_only"))	{
				opts.create_only = true;
			}

	    	$(this).tokenInput(sel_data, opts);

	    });
	
	    $('input.tokenInput:not(.initialized)').each(function() {
	    	var $self = $(this);
	    	opts = {
	    		width: $(this).width()
	    	};
	    	...
	    	if ($(this).val()) {
	    		if ($(this).val().indexOf('{') != -1 ) {
	        		opts.prePopulate = eval( $(this).val()+';' );
	    		}
	    	}
	    	
	    	if ($(this).attr('data-url') && $(this).attr('data-url').substr(0,1) == '[') {
	    		data_or_url = eval($(this).attr('data-url'));
	    	} else {
	    		data_or_url = $(this).attr('data-url');
	    	}
	    	...
	    	if (!$(this).attr('data-default_data') || $(this).attr('data-default_data').length <= 0) {
	    		default_data ='';
	    	} else if ($(this).attr('data-default_data').substr(0,1) == '[') {
	    		default_data = eval($(this).attr('data-default_data'));
	    	} else if ($(this).attr('data-default_data').length > 0) {
	    		default_data = $(this).attr('data-default_data');
	    	}
	    	
	    	if ($(this).attr('onAdd')) {
	    		opts.onAdd = function(data) { eval('"'+$(this).attr('onAdd')+'"'); }
	    	}

	    	if ($(this).attr('onDelte')) {
	    		opts.onDelte = function(data) { eval($(this).attr('onDelte')) }
	    	}

	    	$(this).tokenInput(data_or_url, opts, default_data);
